...
module.exports = function (RED) {
  function QueueGateNode(config) {
    ...
    context.get(["state", "queue"], storeName, function (err, state, queue) {
      ...
      switch (
        state // initialize status display
      ) {
        case "open":
        ...
        case "queueing":
          ...
          queueStatus.shape =
            queue.length < node.maxQueueLength ? "ring" : "dot";
      ...
      context.get(["state", "queue"], storeName, function (err, state, queue) {
            ...
            case node.triggerCmd:
              if (state === "queueing") {
                ...
                if (queue.length > 0) {
                  ...
                  var output = { topic: "control", payload: "trigger" };
                  node.send(output);
                }
              }
              break;
        ...
        context.set(
          ...
          function (err) {
            if (err) {
              node.error(
                "message error writing to context store: " + storeName,
                msg
              );
            }
          }
